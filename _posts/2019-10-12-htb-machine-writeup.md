...
<p>Ok so breaking it down: As root, clear out environment variables, set <code>PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin</code> then execute <code>run-parts</code>. Similarly let's look at run-parts' options. You may already know where I went wrong but let's follow my logic as I stepped through.</p>
